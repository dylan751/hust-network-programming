...
int main(int argc, char **argv)
{
    ...
    if (sfd != INVALID_SOCKET)
    {
        ...
        saddr.sin_port = htons(6101); // Listen ở port 5555
                        ...
                        else if (strncmp(buffer, "GET", 3) == 0)
                        {
                            ...
                            if (port != -1)
                            {
                                ...
                                while (0 == 0)
                                {
                                    ...
                                    int c_cfd = accept(ffd, (SOCKADDR *)&caddr, &clen); // cfd: client file descriptor
                                    if (c_cfd != INVALID_SOCKET)
                                    {
                                        ...
                                        while (!feof(f))
                                        {
                                            ...
                                            SendPacket(c_cfd, buffer, strlen(buffer));
                                        }
                                        fclose(f);
